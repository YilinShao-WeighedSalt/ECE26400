...
// forming a two-dimensional array from a one-dimensional array
...
static int write_maze_to_file(char *filename, maze *my_maze)
{
   ...
   unsigned char (*byte_array)[bytes_per_row] = malloc(sizeof(*byte_array) * my_maze->header.dimensions.row);
   if (byte_array == NULL) {
      fclose(fp);
      return 0;
   }
   ...
   for (i = 0; i < my_maze->header.dimensions.row; i++) {
      ...
      for (j = 0; j < data_per_row; j++) {
         ...
         byte_array[i][j] = 0;  // initialize to 0
         int k;
         for (k = 0; k < 8; k++) {
            int col = j * 8 + k;
            if (col < my_maze->header.dimensions.col) {  // real data
      ...
      for (; j < bytes_per_row; j++) {  // handle the padding bytes
         byte_array[i][j] = 0xff;
      }
   }

   int status = fwrite(byte_array, sizeof(*byte_array), my_maze->header.dimensions.row, fp) == my_maze->header.dimensions.row;
   free(byte_array);
   fclose(fp);
   return status;
}
