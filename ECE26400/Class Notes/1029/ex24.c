// if n = 2m
//    Fib_2m = 2 * Fib_(m-1) * Fib_m + Fib_m * Fib_m
//    Fib_(2m-1) = Fib_m * Fib_m + Fib_(m-1) * Fib_(m-1)
//
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
// fibn is fib(n) and fibn1 is fib(n-1)
//
static void log_Fib_helper(unsigned int n, unsigned long *fibn, unsigned long *fibn1)
{
   ...
   if (half_n % 2 == 0) { // half_n is even
      ...
      // fibhalf_n1 stores fib(half_n - 1), fibhalf_n stores fib(half_n - 1 - 1 = half_n - 2)
      // store fib(half_n) = fib(half_n - 1) + fib(half_n - 2) = fibhalf_n1 + fibhalf_n in fibhalf_n
      fibhalf_n += fibhalf_n1;
   }
   // apply the formulas to compute fibn and fibn1
...
static unsigned long log_Fib(unsigned int n)
{
   ...
   if (n % 2 == 0) { // n is even
      ...
      // use fibn1 to store fib(n-1) and fibn to store fib(n-2)
